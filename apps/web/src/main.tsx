...
function AppShell({ user, onLogout, onUserChange }: { user: CurrentUser; onLogout: () => void; onUserChange: (user: CurrentUser) => void }) {
  ...
  const initialSupportNavigation = ["/escalas", "/pausas", "/campanhas", "/faq", "/fluxos", "/scriptoteca"].includes(window.location.pathname)
  ...
  const startsInFaq = initialSupportNavigation?.view === "faq" && visibleNavByKey.has("faq");
  ...
  const [activeView, setActiveView] = useState<ViewKey>(startsInHelp ? "help" : startsInSchedules ? "supportSchedules" : startsInPauses ? "supportPauses" : startsInCampaigns ? "supportCampaigns" : startsInFaq ? "faq" : startsInServiceFlows ? "serviceFlows" : startsInScriptLibrary ? "scriptLibrary" : startsInWiki ? "wiki" : startsInAnnouncements ? "announcements" : visibleNav[0]?.key ?? "dashboard");
    ...
    <main className={sidebarCollapsed ? "app-frame sidebar-collapsed" : "app-frame"}>
      ...
      <section className="workspace">
        ...
        {activeItem.key === "supportSchedules" ? (
          ...
          <FaqThreadsView user={user} initialStatus={viewIntent.faq?.status} initialThreadId={viewIntent.faq?.threadId} />
...
function App() {
  ...
  const isFaq = window.location.pathname === "/faq" && new URLSearchParams(window.location.search).has("organizationId");
