...
function emptyScriptDraft(categoryId = "") {
  return { categoryId, title: "", channel: "WHATSAPP", body: "", tags: "", status: "DRAFT" };
}
...
export function ScriptLibraryView({ user }: { user: CurrentUser }) {
  ...
  useEffect(() => {
    void load("");
  }, [selectedCategoryId, selectedTag, includeObsolete]);
  ...
  async function saveScript(event: FormEvent) {
    event.preventDefault();
    await run(async () => {
      const payload = { ...scriptDraft, tags: parseTags(scriptDraft.tags) };
            ...
            <div className="wiki-page-list wiki-page-list-paginated">
              {paginatedScripts.map((script) => (
                ...
                >
                  ...
                  <span>{script.category.name} / {script.channel} / {script.status}</span>
            ...
            <>
              <div className="detail-header">
                <div>
                  ...
                  <p className="muted">{selected.channel} · {selected.status}</p>
                </div>
